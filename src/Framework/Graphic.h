...
namespace Framework 
{
	...
	namespace Graphics 
	{
		...
		void ClearRenderTexture(Framework::RenderTexture* rt, const RGBA32 bgColor, UINT clearFlag = D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL);
		void Present();
		...
		void Blit(ID3D11ShaderResourceView* srv, RenderTexture* dst);
		void Blit(RenderTexture* src, RenderTexture* dst);
		void Blit(Texture* texture, RenderTexture* dst);
	}
}
